...
#| tbl-cap: Locations with too few detailed surveys during the last 24 years
...
#| tbl-cap: Locations with too few detailed surveys during the last 12 years
...
```{r leaflet}
#| fig.cap: "Map with usable locations during the last 12 years."
section_total |>
  ...
  mutate(
    useable = factor(
      ...
      levels = c(FALSE, TRUE), labels = c("not usable", "usable")
    )
  ) |>
  inner_join(locations, by = c("location_id" = "id")) |>
...
pal_use <- colorFactor(
  palette = c(inbo_geel, inbo_hoofd), levels = c("not usable", "usable")
)
leaflet(effort_12) |>
  addTiles() |>
  addCircleMarkers(
    color = ~pal_use(useable), fillColor = ~pal_detailed(detailed)
  ) |>
  addLegend(
    position = "bottomright", pal = pal_detailed, values = ~detailed,
    title = "number of\ndetailed surveys"
  ) |>
  addLegend(
    position = "bottomright", pal = pal_use, values = ~useable,
    title = "usable locations"
  )
```

...
osm_tile <- function(
  ...
  extra_zoom = 0,
  cache_path = R_user_dir("batanalysis", which = "cache"),
  type = c("osm", "water")
) {
  assert_that(
    inherits(bbox, c("bbox", "numeric")),
    has_name(bbox, "xmin"),
    has_name(bbox, "xmax"),
    has_name(bbox, "ymin"),
    has_name(bbox, "ymax"),
    ...
    bbox["ymax"] < 90,
    noNA(bbox),
    is.number(extra_zoom),
    is.string(cache_path),
    noNA(cache_path)
  )
  type <- match.arg(type)
  ...
  expand.grid(
    ...
    mutate(
      ...
      cache = file.path(cache_path, gsub("https://.*?/(.*)", "\\1", .data$url)),
